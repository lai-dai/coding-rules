...
import {
  useCallbackRef,
} from "~/shared/hooks/state/use-callback-ref"
...
export function useLocalStorage<T>(props: UseLocalStorageProps<T>,): UseLocalStorageReturn<T> {
  const {
    initialValue, key, maxAge = 0, version = 0, ...options
  } = props
  ...
  const deserializer: (value: string) => ClientState<T> = useCallbackRef(typeof options?.raw === "boolean"
    ? options.raw === false
    ...
    try {
      ...
      if (initialValue) {
        localStorage.setItem(
          key, serializer(formatValue(initialValue))
        )
      }
      return initialValue
    }
  ...
  const [
    state,
    setState,
  ] = React.useState(() => initializer.current(key))

  React.useLayoutEffect(
    () => setState(initializer.current(key)), [key]
  )
